...
table(example_adat$Sex)

#Find the age composition of the dataset
summary(example_adat$Age)

#Are there NAs in the dataset?
table(is.na(example_adat$Sex))
...
length(table(example_olink$UniProt))

#How many samples are there?

length(table(example_olink$SampleID))
...
newdf <- df[df$quartile105 == "Q1"|df$quartile105 == "Q4",]

t.test(Age ~ quartile105, newdf)
...
df$quartile104 <-  cut(df$seq.10464.6,
                   breaks = quantile(df$seq.10464.6, probs = c(0, 0.25, 0.5, 0.75, 1), na.rm = TRUE),
                   include.lowest = TRUE,
                   labels = c("Q1", "Q2", "Q3", "Q4"))


chisq.test(df$quartile104,df$Sex)

table(df$quartile104,df$Sex)
...
multivariate_1052 <- lm(scaled_1052 ~ Age + Sex, df)
#Read out the summary
summary(multivariate_1052)
```
...
```{r}
qqnorm(residuals(multivariate_1052))
qqline(residuals(multivariate_1052))
```
...
df$scaled_1043 <- scale(df$seq.10438.19)
df$scaled_1043 <- ifelse(df$scaled_1043 > 4|df$scaled_1043 < -4, NA, df$scaled_1043)
multi_linear_1043 <- lm(scaled_1043 ~ Age + Sex + Age*Sex, df)
summary(multi_linear_1043)

#Also, try without the interaction term and examine what happens to the significance. Why do you think that occurred?

multi_linear_1043 <- lm(scaled_1043 ~ Age + Sex, df)
summ_multi <- summary(multi_linear_1043)

summ_multi$coefficients
```
...
table(metrics_df$Age_pval_FDR < 0.05)
...
metrics_df$Age_FDR_Sig <- metrics_df$Age_pval_FDR < 0.05
#Re-draw the ggplot

ggplot(metrics_df, aes(x = Age_beta, y = -log(Age_pval, base = 10), colour = Age_FDR_Sig)) + geom_point(alpha = 0.5)
```
...
metrics_df$Sex_pval_FDR <- p.adjust(metrics_df$Sex_pval, method = "fdr")
metrics_df$Age_Sex_pval_FDR <- p.adjust(metrics_df$Age_Sex_pval, method = "fdr")

metrics_df$Sex_FDR_Sig <- metrics_df$Sex_pval_FDR < 0.05
metrics_df$Age_Sex_FDR_Sig <- metrics_df$Age_Sex_pval_FDR < 0.05

min(metrics_df$Age_Sex_pval)



ggplot(metrics_df, aes(x = Sex_beta, y = -log(Sex_pval, base = 10), colour = Sex_FDR_Sig)) + geom_point(alpha = 0.5)

ggplot(metrics_df, aes(x = Age_Sex_beta, y = -log(Age_Sex_pval, base = 10), colour = Age_Sex_FDR_Sig)) + geom_point(alpha = 0.5)
```
...
dfkey <- df %>% select(c("SampleId","seq.10527.22"))

if (!requireNamespace("RNOmni")) {
    install.packages("RNOmni")
  }
library(RNOmni)

dfkey$INT <- RankNorm(dfkey$seq.10527.22)
...
which.min(Add_result$P)

Add_result$BETA[7338]
```
